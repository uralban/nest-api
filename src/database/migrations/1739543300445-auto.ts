...
export class Auto1739543300445 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."company_visibility_enum" AS ENUM('visible', 'hidden')`,
    );
    await queryRunner.query(
      `ALTER TABLE "company" ADD "visibility" "public"."company_visibility_enum" NOT NULL DEFAULT 'hidden'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "company" DROP COLUMN "userEmailLogin"`,
    );
